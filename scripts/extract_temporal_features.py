#!/usr/bin/env python3
"""Extract frozen temporal EEG/audio features from development rows only."""
...
def main() -> None:
    ...
    allowed_roles = {
        config["resampling"]["development_role"],
        config["resampling"]["validation_role"],
    }
    manifest = manifest.loc[
        manifest["model_eligible"] & manifest["subset_role"].isin(allowed_roles)
    ].copy()
    if set(manifest["subset_role"]) != allowed_roles:
        raise ValueError("development manifest must contain both frozen roles")
    ...
    print(
        f"wrote {len(metadata)} development rows: EEG {eeg.shape}, "
